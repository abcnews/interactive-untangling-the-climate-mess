...
window["KeyshapeJS"] = (function () {
  ...
  function propertyFlags(prop) {
    ...
    if (!flags) {
      ...
      if (isSet(allCssProps[prop])) {
        ...
        flags |=
          prop.toLowerCase().indexOf("color") == prop.length - 5
            ? FLAG_TYPE_COLOR
            : FLAG_TYPE_STRING;
      } else {
  ...
  function cubicBezierY(cbdata, x) {
    ...
    return (
      /*(1-t)*(1-t)*(1-t)*0 +*/ 3 * t * (1 - t) * (1 - t) * cbdata[2] +
      3 * t * t * (1 - t) * cbdata[4] +
      t * t * t
    );
  }
  ...
  function interpolate(type, v1, v2, t) {
    if (type == FLAG_TYPE_NUMBER || type == FLAG_TYPE_LENGTH) {
      // number or length
      return (v2 - v1) * t + v1;
    }
    ...
    if (type == FLAG_TYPE_FILTER) {
      ...
      while (i < reslen) {
        ...
        if (v1[i] == FILTER_DROPSHADOW) {
          // multiple values and color for drop-shadow
          ...
          res[i + 4] =
            ((clampColor(g) << 16) | (clampColor(b) << 8) | clampColor(a)) +
            (clampColor(r) | 0) * 16777216;
          i += 3;
        }
        i += 2;
      }
  ...
  function updateAllAnimations(mainUpdate) {
    if (!mainUpdate && isTicking) {
      // no update if mainloop will do it
      return;
    }
  ...
  function startTicking() {
    if (!isTicking) {
      isTicking = true;
      startAnimating(FRAMES_PER_SECOND); // Add on to set FPS
      //   reqAnimationFrame(mainloop);
    }
  }
  ...
  function composeAndSetTransformAttr(elem) {
    ...
    if (mpath) {
      ...
      if (dist < 0) {
        dist = 0;
      } // old SVG1.1 browsers don't clamp getPointAtLength(dist)
      if (dist > 100) {
        dist = 100;
      }
      ...
      if (mpath[MPATH_AUTOROTATE]) {
        ...
        var prevpt;
        if (dist < 0.5) {
          // tangent of start point 0 and point at 0.5
          prevpt = pt;
          pt = mpath[MPATH_ELEMENT].getPointAtLength(0.5);
        } else {
          // get point before dist
          prevpt = mpath[MPATH_ELEMENT].getPointAtLength(dist - 0.5);
        }
        var deg =
          (Math.atan2(pt.y - prevpt.y, pt.x - prevpt.x) * 180) / Math.PI;
    ...
    for (var prop = P_POSITIONX; prop < trans.length; ++prop) {
      ...
      if (val != defaultVal) {
        ...
        } else if (prop >= P_ANCHORX) {
          // anchor x and y
          transform += prop == P_ANCHORX ? val + ",0" : "0," + val;
        } else if (prop >= P_SCALEX) {
          // scale x and y
  ...
  KsAnimation.prototype = {
    _updateFinishedState: function (didSeek) {
      ...
      if (this._startTimeVal !== null) {
        ...
        if (
          this._playRate > 0 &&
          curTime !== null &&
          curTime >= this._rangeOut
        ) {
          if (this._loopCount) {
            // loop jumping to range in
            this._startTimeVal =
              sysTimelineTime - this._rangeIn / this._playRate;
          ...
          this._playRate < 0 &&
          curTime !== null &&
          curTime <= this._rangeIn
        ) {
          if (this._loopCount && this._rangeOut != Infinity) {
            // loop jumping to range out
            this._startTimeVal =
              sysTimelineTime - this._rangeOut / this._playRate;
            this._loopCount--; // this doesn't decrement Infinity
            pendingCallback = PENDING_ONLOOP;
          } else {
            // no looping
          ...
          if (didSeek && this._holdTime !== null) {
            ...
            // time is set
            this._startTimeVal =
              sysTimelineTime - this._holdTime / this._playRate;
          }
          this._holdTime = null;
        }
      }
    ...
    _performAutoRemove: function () {
      if (
        this._options["autoremove"] !== false &&
        this._getState() == STATE_FINISHED
      ) {
        remove(this);
      }
    },
    ...
    _parseTime(value) {
      ...
      if (
        !isSet(this._options["markers"]) ||
        !isSet(this._options["markers"][value])
      ) {
    ...
    play: function (millisecs) {
      if (isSet(millisecs) && millisecs !== null) {
        ...
        if (this._playRate < 0 && millisecs < this._rangeIn) {
          millisecs = this._rangeIn;
        }
        if (this._playRate > 0 && millisecs > this._rangeOut) {
          millisecs = this._rangeOut;
        }
    ...
    loop: function (loopCount) {
      ...
      if (this._loopCount < 0 || isNaN(this._loopCount)) {
        this._loopCount = 0;
      }
      return this;
    },
    ...
    _getState: function () {
      var curTime = this._getCurrentTime();
      if (this._hasPendingPlay) {
        // additional condition, there is no "pending" state
      ...
      if (
        (this._playRate > 0 && curTime >= this._rangeOut) ||
    ...
    marker: function (name) {
      return isSet(this._options["markers"])
        ? this._options["markers"][name]
        : undefined;
    },
  ...
  function parseTimingFn(val) {
    if (Array.isArray(val)) {
      // array means a shorthand timing function
      return val;
    }
    if (startsWith(val, "cubic-bezier(")) {
      ...
      return [
        svals[1] && svals[1].trim() == "start"
          ? TIMING_FN_STEP_START
          : TIMING_FN_STEP_END,
        parseFloat(svals[0])
      ];
    }
    ...
    if (startsWith(cstr, "rgba(")) {
      ...
      return (
        ((parseInt10(p[0]) << 24) +
          ...
          parseFloat(p[3]) * 255) <<
        0
      ); // shift alpha by 0 to convert it to int
    }
    return cstr;
  }
  ...
  function parseValues(prop, values) {
    ...
    for (var i = 0; i < values.length; ++i) {
      if (type == FLAG_TYPE_PATH) {
        ...
        var commands = val.match(/[A-DF-Za-df-z][-+0-9eE., ]*/gi);
        var s = [];
        for (var k = 0; k < commands.length; ++k) {
          s.push(commands[k][0]);
          var coords =
            commands[k].trim().length > 1
        ...
        if (startsWith(values[i], "#")) {
          ...
          if (!hasAlpha) {
            // add alpha to colors without it
    ...
    for (var ai = 0; ai < tlData.length - 1; ai += 2) {
      ...
      for (var i = 0; i < anim.length; ++i) {
        ...
        if (isSet(motionPath) && prop === P_MOTION_DISTANCE) {
          ...
          var mpathElement = document.createElementNS(
  ...
  function animate() {
    ...
    if (arguments.length % 2 == 1) {
      // odd count of arguments means that options are given
